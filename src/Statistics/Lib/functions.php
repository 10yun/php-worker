...
{
    \streetlamp\Statistics\Lib\Cache::$lastSuccessIpArray = array();
    ...
    {
        list($ip, $port) = explode(':', $address);
        \streetlamp\Statistics\Lib\Cache::$lastSuccessIpArray[$ip] = $ip;
    }

     \streetlamp\Statistics\Lib\Cache::$lastFailedIpArray = array_diff($ip_list,  \streetlamp\Statistics\Lib\Cache::$lastSuccessIpArray);
...
function check_auth()
{
    ...
    if(streetlamp\Statistics\Config\Config::$adminName == '' && streetlamp\Statistics\Config\Config::$adminPassword == '')
    {
        return true;
    }
    // 进入验证流程
    ...
    if(!isset($_SESSION['admin']))
    {
        ...
        else 
        {
            ...
            if($admin_name != streetlamp\Statistics\Config\Config::$adminName || $admin_password != streetlamp\Statistics\Config\Config::$adminPassword)
            {
                $msg = "用户名或者密码不正确";
                include ST_ROOT . '/Views/login.tpl.php';
                _exit();
