 ...
 */
namespace streetlamp\Statistics\Modules;
function main($module, $interface, $date, $start_time, $offset)
{
    ...
    $all_st_str = '';
    if(is_array(\streetlamp\Statistics\Lib\Cache::$statisticDataCache['statistic']))
    {
        foreach(\streetlamp\Statistics\Lib\Cache::$statisticDataCache['statistic'] as $ip=>$st_str)
        {
            $all_st_str .= $st_str;
        }
    }
    ...
    if( \streetlamp\Statistics\Lib\Cache::$lastFailedIpArray)
    {
        $err_msg = '<strong>无法从以下数据源获取数据:</strong>';
        foreach (\streetlamp\Statistics\Lib\Cache::$lastFailedIpArray as $ip)
        {
            $err_msg .= $ip.'::'.\streetlamp\Statistics\Config\Config::$ProviderPort . '&nbsp;';
        }
    }
    
    if(empty(\streetlamp\Statistics\Lib\Cache::$ServerIpList))
    {
        $notice_msg = <<<EOT
<h4>数据源为空</h4>
...
{
    \streetlamp\Statistics\Lib\Cache::$statisticDataCache['statistic'] = array();
    $buffer = json_encode(array('cmd'=>'get_statistic','module'=>$module, 'interface'=>$interface, 'date'=>$date))."\n";
    $ip_list = (!empty($_GET['ip']) && is_array($_GET['ip'])) ? $_GET['ip'] : \streetlamp\Statistics\Lib\Cache::$ServerIpList;
    $reqest_buffer_array = array();
    $port = \streetlamp\Statistics\Config\Config::$ProviderPort;;
    ...
    foreach($read_buffer_array as $address => $buf)
    {
        ...
        {
            if(!isset(\streetlamp\Statistics\Lib\Cache::$modulesDataCache[$mod]))
            {
                \streetlamp\Statistics\Lib\Cache::$modulesDataCache[$mod] = array();
            }
            foreach($interfaces as $if)
            {
                \streetlamp\Statistics\Lib\Cache::$modulesDataCache[$mod][$if] = $if;
            }
        }
        \streetlamp\Statistics\Lib\Cache::$statisticDataCache['statistic'][$ip] = $statistic_data;
    }
}
