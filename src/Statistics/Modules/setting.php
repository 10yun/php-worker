    ...
    switch($act)
    {
        case 'save':
           ...
           if($detect_port<0 || $detect_port > 65535)
           {
            ...
            $suc_msg = "保存成功";
            \streetlamp\Statistics\Config\Config::$ProviderPort = $detect_port;
            saveDetectPortToCache();
            break;
        default:
            $detect_port = \streetlamp\Statistics\Config\Config::$ProviderPort;
    }
...
function saveDetectPortToCache()
{
    ...
    file_put_contents(ST_ROOT . '/Config/Cache/'.time().'.detect_port.cache.php', "<?php\n\\streetlamp\\Statistics\\Config\\Config::\$ProviderPort=".var_export(\streetlamp\Statistics\Config::$ProviderPort,true).';');
}
