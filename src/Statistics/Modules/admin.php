...
function admin()
{
    ...
    switch($act)
    {
        case 'detect_server':
            ...
            // 广播
            socket_sendto($socket, $buffer, strlen($buffer), 0, '255.255.255.255', \streetlamp\Statistics\Config\Config::$ProviderPort);
            // 超时相关
            ...
            {
                if(!isset(\streetlamp\Statistics\Lib\Cache::$ServerIpList[$ip]))
                {
                    $ip_list_str .= $ip."\r\n";
                    $count ++;
                }
        ...
        case 'add_to_server_list':
            ...
            if($ip_list)
            {
                foreach($ip_list as $ip)
                {
                    ...
                    if(false !== ip2long($ip))
                    {
                        \streetlamp\Statistics\Lib\Cache::$ServerIpList[$ip] = $ip;
                    }
                }
            }
            $suc_msg = "添加成功";
            foreach(\streetlamp\Statistics\Lib\Cache::$ServerIpList as $ip)
            {
                $ip_list_str .= $ip."\r\n";
            }
        ...
        case 'save_server_list':
            ...
            \streetlamp\Statistics\Lib\Cache::$ServerIpList = array();
            $ip_list = explode("\n", $_POST['ip_list']);
            if($ip_list)
            {
                foreach($ip_list as $ip)
...
function saveServerIpListToCache()
{
    ...
    file_put_contents(ST_ROOT . '/Config/Cache/'.time().'.iplist.cache.php', "<?php\n\\streetlamp\\Statistics\\Lib\\Cache::\$ServerIpList=".var_export(\streetlamp\Statistics\Lib\Cache::$ServerIpList,true).';');
}
